...
async function initStartupScene() {
  ...
  const textureRoot = new URL("../assets/startup-3d/", import.meta.url);
  const textures = {
...
function render(state, timeMs) {
  const { gl } = state;
  const isMenu = titleScreen && !titleScreen.classList.contains("is-hidden");
...
if (canvas && sceneEl && titleScreen && !lowPowerStartup.matches) {
